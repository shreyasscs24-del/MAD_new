...
class _GoalsScreenState extends State<GoalsScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return SafeArea(child: SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _buildGoalCard(emoji: '🚫', title: 'Limit Instagram', subtitle: 'Target: 45 min/day · Today: 1h 28m used', progress: 0.62, progressColor: AppColors.red, percentage: '62%', percentageColor: AppColors.red, statusText: 'Over limit today', statusColor: AppColors.red, statusBgColor: AppColors.red.withValues(alpha: 0.15), streak: '3 day streak'),
        const SizedBox(height: 12),
        _buildGoalCard(emoji: '📖', title: 'Read 20 pages/day', subtitle: '16 pages done · 4 remaining', progress: 0.80, progressColor: AppColors.purple, percentage: '80%', percentageColor: AppColors.purple, statusText: 'On track', statusColor: AppColors.green, statusBgColor: AppColors.green.withValues(alpha: 0.15), streak: '12 day streak 🔥'),
        const SizedBox(height: 12),
        _buildGoalCard(emoji: '💻', title: '1 Code Lesson/day', subtitle: 'Not started yet · 8h left today', progress: 0.0, progressColor: AppColors.textMuted, percentage: '0%', percentageColor: AppColors.textMuted, statusText: 'Not started', statusColor: AppColors.red, statusBgColor: AppColors.red.withValues(alpha: 0.15), streak: '0 streak'),
        const SizedBox(height: 12),
        _buildGoalCard(emoji: '🧠', title: '10 Vocab Words/day', subtitle: '4 words done · 6 remaining', progress: 0.40, progressColor: AppColors.orange, percentage: '40%', percentageColor: AppColors.orange, statusText: 'In progress', statusColor: AppColors.orange, statusBgColor: AppColors.orange.withValues(alpha: 0.15), streak: '7 day streak'),
  ...
  Widget _buildGoalCard({required String emoji, required String title, required String subtitle, required double progress, required Color progressColor, required String percentage, required Color percentageColor, required String statusText, required Color statusColor, required Color statusBgColor, required String streak}) {
    return Container(width: double.infinity, padding: const EdgeInsets.all(16),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(percentage, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: percentageColor)),
        ]),
        const SizedBox(height: 6),
        Text(subtitle, style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
        const SizedBox(height: 10),
        ClipRRect(borderRadius: BorderRadius.circular(5),
          child: LinearProgressIndicator(value: progress, minHeight: 8, backgroundColor: AppColors.surfaceLight, valueColor: AlwaysStoppedAnimation<Color>(progressColor))),
        const SizedBox(height: 10),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Text(streak, style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
        ]),
      ]));
  }
